...
type shape = 
  ...
  | Ellipse of {color: color; p1: point; p2: point; t: thickness}
...
let repaint (g: gctx) : unit =
  let draw_shape (s: shape) : unit =
    begin match s with
      ...
      | Points ps -> Gctx.draw_points (with_params g ps.color ps.t)
                       (ps.points)   
      | Ellipse e -> let (x, y) = e.p1 in 
        let (x2, y2) = e.p2 in
        (** avg position based on the first 2 points*)
        let (x3, y3) = ((x + x2)/2, (y + y2)/2) in 
        (** the *)
        let rx = abs (x2 - x3) in 
        let ry = abs (y2 - y3) in 
        Gctx.draw_ellipse (with_params g e.color e.t) 
          (x3, y3) rx ry 
    end in 
  let draw_preview (so: shape option) : unit =
    begin match so with
      ...
      | Some Points ps -> Gctx.draw_points (with_params g ps.color ps.t)
                            (ps.points)   
      | Some Ellipse e -> let (x, y) = e.p1 in 
        let (x2, y2) = e.p2 in
        let (x3, y3) = ((x + x2)/2, (y + y2)/2) in 
        let rx = abs (x2 - x3) in 
        let ry = abs (y2 - y3) in 
  ...
  begin match (event_type event) with
    | MouseDown ->
      ...
      (begin match paint.mode with 
         ...
         | PointMode -> 
           let ps = Points {color = paint.color; points = [p]; 
      ...
      (begin match paint.preview with
         | None -> 
           (begin match paint.mode with 
              | LineStartMode -> () 
              | LineEndMode p1 ->
                let s : shape = Line {color=paint.color; p1=p1; p2=p; 
                                      t=paint.thickness} in   
                paint.preview <- Some s 
              | PointMode -> () 
              | EllipseStartMode -> ()
              | EllipseEndMode p1 -> 
                let s : shape = Ellipse {color=paint.color; p1=p1; p2=p; 
                                         t=paint.thickness} in  
                paint.preview <- Some s 
            end)
         | Some s ->
           (begin match s with 
              | Line l -> let s2 : shape = Line {color=paint.color; p1=l.p1;
                                                 p2=p; t=paint.thickness} in
                          paint.preview <- Some s2;
                          repaint (gc)
              | Points ps -> let points_list = ps.points @ [p] in 
                let new_ps = Points {color = paint.color; 
                                     points = points_list;
                                     t = paint.thickness} in 
                             paint.preview <- Some new_ps; 
                             repaint (gc)
              | Ellipse e -> let s2 : shape = Ellipse {color=paint.color; 
                                                       p1=e.p1; p2=p; 
                                                       t=paint.thickness} in
                             paint.preview <- Some s2;
                             repaint (gc)
    ...
    | MouseUp ->
      ...
      (begin match paint.mode with  
         | LineStartMode -> () 
         | LineEndMode p1 -> (begin match paint.preview with 
             | None -> ()
             | Some s2 -> Deque.insert_tail (s2) paint.shapes;
                          paint.preview <- None; 
                          paint.mode <- LineStartMode 
           end) 
         | PointMode -> (begin match paint.preview with 
             | None -> ()
             | Some ps -> Deque.insert_tail (ps) paint.shapes;
                          paint.preview <- None
           end) 
         | EllipseStartMode -> () 
         | EllipseEndMode p1 -> (begin match paint.preview with 
             | None -> ()
             | Some s2 -> Deque.insert_tail(s2)paint.shapes;
                          paint.preview <- None; 
...
(** function switches paint.mode to LineStartMode*)
let line () : unit =
  paint.mode <- LineStartMode 

;; nc_line.add_event_listener (mouseclick_listener line)

(** function switches paint.mode to PointMode*)
let point () : unit =
...
(** function to switch between thick and thin *)
let switch () : unit = 
  if (vc_checkbox.get_value ()) then paint.thickness <- thin
  else paint.thickness <- thick  

;; vc_checkbox.add_change_listener (fun x -> switch())


(** Create thickness slider *)
let (w_slider, vc_slider) = slider (0, 0) "Line thickness"

(** Function to adjust thickness once checkbox is already checked *)
let slider_action ((x,y) : int * int) : unit = 
  if (vc_checkbox.get_value()) then 
    (** minimum thickness is thick since thick lines is checked *)
    if x <= 0 then paint.thickness <- thick 
    (** increase the thickness based on the x position of the mouse *)
    else if (x >= 200) || (x>0) then paint.thickness <- thick + x
    else ()  
  else () 

;; vc_slider.add_change_listener (fun (a,b) -> slider_action (a,b))
...
let mode_toolbar : widget = 
  let ws : widget list = [w_point; spacer; 
                          w_line; spacer; 
                          w_ellipse; spacer; 
                          w_undo; spacer;
                          w_checkbox; spacer;
...
let paint_widget =
  let ws : widget list = [paint_canvas; mode_toolbar; color_toolbar] in 
  Widget.vlist (ws)
